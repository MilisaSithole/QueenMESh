...
- **Tier 2 (Medium).** Requires noticing that a region is entirely confined to one or two rows/columns, which then rules out those rows/columns for every *other* region (the Queens equivalent of Sudoku's "pointing pairs"). Puzzles that need at least one Tier-2 deduction, but nothing beyond it, are Medium.
  ...
  **Correction from Phase 5.2: "one or two" is wrong as stated, and implementing it literally would be unsound.** A *single* region confined to *two* rows rules out nothing — its crown takes one of the two rows and leaves the other free for anybody. The sound generalisation is **k groups whose candidates span exactly k lines**, which consumes those lines. k=1 is the genuine pointing case (one region in one row); k≥2 is the multi-region case the plan describes under Tier 3. So Tier 2 is implemented as k=1 in all six axis directions (region→row, region→column, row→region, column→region, row→column, column→row), plus one Queens-specific rule: a cell can be eliminated when every remaining candidate of some group touches it, since a crown there would leave that group nowhere legal to go. Tier 3 is the same locked-set machinery at k=2 and k=3.
